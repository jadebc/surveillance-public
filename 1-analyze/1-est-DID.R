...
#-----------------------------------------
# estimate parameters - primary analysis

# Census data subset by zipcode
# Adjust for race

# PRIMARY ANALYSIS  - additive scale DID
#-----------------------------------------
...
# PRIMARY ANALYSIS  - relative scale DID
#-----------------------------------------
data_list = unlist(data_zip_race, recursive=FALSE)
...
res_zip_race_rdid = bind_rows(res_list) 
...
for(i in 1:length(data_list)) {
  res_list[[i]] = bind_rows(apply(seas_list, 1, function(x)
    fit_did_glm_seas(
      ...
      offset = "logN",
      covariates = "agecat",
      label = "ACS data, adjusted for age"
    )))
...
#-----------------------------------------
results = bind_rows(res_zip_race,
                    res_zip_race_rdid,
                    res_zip, 
                     res_zip_fpsens,
                     res_dist,
...
saveRDS(results, file=paste0(res_dir, "flu_surv_differences.RDS"))
